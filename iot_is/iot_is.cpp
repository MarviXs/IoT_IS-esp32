...
void IoTIs::connect_task(void *pvParameters)
{
    ...
    mqtt_cfg.broker.address.hostname = instance->_mqttHost.c_str();
    mqtt_cfg.broker.address.port = instance->_mqttPort;
    mqtt_cfg.broker.address.transport = MQTT_TRANSPORT_OVER_TCP;

    mqtt_cfg.buffer.size = 16384;
    mqtt_cfg.session.disable_clean_session = true;

    mqtt_cfg.credentials.client_id = instance->_accessToken.c_str();
    mqtt_cfg.credentials.username = instance->_accessToken.c_str();
...
void IoTIs::process_received_job(esp_mqtt_event_handle_t event)
{
    ...
    ESP_LOGI(TAG, "Received job: %s with %ld steps and %ld cycles", jobFbs.name.c_str(), jobFbs.total_steps, jobFbs.total_cycles);
